...
async function fetchDevOtpCode(request: import('@playwright/test').APIRequestContext) {
...
async function createTestTicket(
  ...
  if (!adminToken) throw new Error('Missing adminToken in localStorage after login');
  ...
  const json = await res.json();
  if (!res.ok()) {
    throw new Error(`Failed to create ticket: ${res.status()} ${JSON.stringify(json)}`);
  }
...
test('Admin ticket detail toggles related data', async ({ page, request }) => {
  ...
  await createTestTicket(page, subject, 'Automated test ticket for detail view');
  ...
  await Promise.all([page.waitForURL(/.*\/admin\/tickets\/[^/]+$/), detailsLink.click()]);
  ...
  await Promise.all([
    page.waitForResponse((r) => r.url().includes('/api/support/admin/tickets/')),
