          ...
          <div className="flex items-start">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-2 text-sm text-green-800 dark:text-green-200">
                ...
                <p>
                  <strong>Total:</strong> {withdrawalDetails.totalAmount.toFixed(8)}{' '}
                  {withdrawalDetails.cryptoType}
                </p>
                <p>
                  <strong>Destination:</strong>{' '}
                  <code className="text-xs break-all">{withdrawalDetails.walletAddress}</code>
                </p>
                <p>
              ...
              <div className="mt-4 p-4 bg-white dark:bg-gray-800 rounded border border-green-200 dark:border-green-700">
                <p className="text-xs text-gray-600 dark:text-gray-400">
                  Your crypto has been locked and is pending admin approval. You&apos;ll be notified
      ...
      >
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      Balance:{' '}
                      {loadingBalances ? '...' : balance?.balance.toFixed(8) || '0.00000000'}
                    </div>
                  </div>
                </button>
              );
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Minimum: {minWithdrawal} {cryptoType} | Available: {currentBalance.toFixed(8)}{' '}
            {cryptoType}
          </p>
        </div>
        ...
        {amount && parseFloat(amount) >= minWithdrawal && (
          <div
            className={`p-4 rounded-lg border ${hasInsufficientBalance ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800'}`}
          >
            <h3
              className={`text-sm font-semibold mb-3 ${hasInsufficientBalance ? 'text-red-900 dark:text-red-100' : 'text-indigo-900 dark:text-indigo-100'}`}
            >
              Withdrawal Summary
            </h3>
              ...
              {hasInsufficientBalance && (
                <div className="pt-2 border-t border-red-300 dark:border-red-700">
                  <p className="text-red-800 dark:text-red-200 text-xs font-semibold">
                    ⚠️ Insufficient balance! You need {total.toFixed(8)} {cryptoType} but only have{' '}
                    {currentBalance.toFixed(8)} {cryptoType}
                  </p>
                </div>
              )}
