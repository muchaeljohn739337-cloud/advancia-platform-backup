...
export default function RewardsDashboard({ userId }: RewardsDashboardProps) {
  ...
  const fetchRewards = useCallback(async () => {
    try {
      const res = await fetch(
        `http://localhost:4000/api/rewards/pending/${userId}`,
      );
  ...
  const fetchTierInfo = useCallback(async () => {
    try {
      const res = await fetch(
        `http://localhost:4000/api/rewards/tier/${userId}`,
      );
      const data = await res.json();
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      const res = await fetch(
        "http://localhost:4000/api/rewards/leaderboard?limit=10",
      );
      const data = await res.json();
  ...
  const handleCheckIn = async () => {
    try {
      const res = await fetch(
        "http://localhost:4000/api/rewards/streak/update",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
