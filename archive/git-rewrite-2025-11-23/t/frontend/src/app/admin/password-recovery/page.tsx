...
export default function PasswordRecoveryAdmin() {
  ...
  const fetchResetRequests = async (status = "pending") => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/password-recovery/admin/requests?status=${status}`,
        {
  ...
  const resetUserPassword = async () => {
    ...
    if (
      !confirm(
        `Reset password for user ${resetUserId}? The new password will be: ${newPassword}`,
      )
    ) {
      return;
    }
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        "/api/password-recovery/admin/reset-user-password",
        {
          method: "POST",
          headers: {
