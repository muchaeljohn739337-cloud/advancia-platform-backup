...
export type EventHandlers = Required<{
  [K in keyof BotHandlers["eventHandlers"]]: NonNullable<
    BotHandlers["eventHandlers"][K]
  >[number];
}>;
export type MessageHandlers = Required<{
  [K in keyof BotHandlers["messageHandlers"]]: NonNullable<
    BotHandlers["messageHandlers"][K]
  >[number];
}>;
export type WorkflowHandlers = {
  [TWorkflowName in keyof Required<
    BotHandlers["workflowHandlers"][keyof BotHandlers["workflowHandlers"]]
  >]: Required<
    BotHandlers["workflowHandlers"][keyof BotHandlers["workflowHandlers"]]
  >[TWorkflowName] extends { handler: infer U }[]
    ? U
    : never;
};
