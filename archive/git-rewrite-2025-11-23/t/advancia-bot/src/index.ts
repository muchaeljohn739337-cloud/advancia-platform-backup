...
**Need more help?** Type "support" to create a ticket!`,
  ...
  // Trump Coin Queries
  if (
    userMessage.includes("trump") ||
    userMessage.includes("coin") ||
...
**Want to Trade?** Visit your dashboard to start!`,
  ...
  // Med-Bed Analytics
  if (
    userMessage.includes("med") ||
    userMessage.includes("bed") ||
...
Would you like to check your current balance? (I can fetch it if you're logged in)`,
  ...
  // Account Recovery
  if (
    userMessage.includes("recover") ||
    userMessage.includes("reset") ||
    userMessage.includes("forgot") ||
    userMessage.includes("locked")
  ) {
    await client.createMessage({
      type: "text",
      conversationId: client.conversationId,
...
**Still Having Issues?** Type "support" for personalized help!`,
  ...
  // KYC Verification
  if (
    userMessage.includes("kyc") ||
    userMessage.includes("verify") ||
    userMessage.includes("verification") ||
    userMessage.includes("identity")
  ) {
    await client.createMessage({
      type: "text",
      conversationId: client.conversationId,
