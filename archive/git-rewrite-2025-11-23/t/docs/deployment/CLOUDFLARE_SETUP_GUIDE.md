...
const ALLOWED_ORIGINS = ["https://advancia.app", "https://www.advancia.app", "https://admin.advancia.app"];
...
async function handleRequest(request) {
  ...
  try {
    ...
    if (origin && ALLOWED_ORIGINS.includes(origin)) {
      ...
      modifiedResponse.headers.set("Access-Control-Allow-Headers", "Content-Type, Authorization, X-API-Key, X-Admin-Key");
    }
...
```tsx
<Script src="https://static.cloudflareinsights.com/beacon.min.js" data-cf-beacon='{"token": "YOUR_TOKEN_HERE"}' strategy="afterInteractive" />
```
