...
router.post("/admin/:id/confirm-escrow", authenticateToken, requireAdmin, async (req, res) => {
...
router.post("/admin/:id/send-to-user", authenticateToken, requireAdmin, async (req, res) => {
...
router.post("/webhook/deposit", async (req, res) => {
  ...
  if (confirmations >= 3) {
    const balanceField = cryptoType === "BTC" ? "btcBalance" : cryptoType === "ETH" ? "ethBalance" : "usdtBalance";
