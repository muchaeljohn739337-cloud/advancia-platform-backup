...
async function main() {
  try {
    ...
    if (!testUser) {
      testUser = await prisma.user.create({
        data: {
          ...
          firstName: "Test",
          lastName: "User",
          usdBalance: 500.0, // $500 balance for testing
          role: "user",
          createdAt: new Date(),
        },
      });
    ...
    if (existingOrders === 0) {
      ...
      const order1 = await prisma.cryptoOrder.create({
        data: {
          ...
          processingFee: 2.5,
          totalUsd: 102.5, // includes 2.5% fee
          status: "pending",
      ...
      console.log(
        `   Order 1: ${order1.cryptoAmount} BTC ($${order1.usdAmount} + fees)`,
      );
      console.log(
        `   Order 2: ${order2.cryptoAmount} ETH ($${order2.usdAmount} + fees)`,
      );
    ...
    if (settings) {
      console.log("\n✅ Admin Settings:");
      console.log(
        `   BTC Address: ${settings.btcAddress ? settings.btcAddress.substring(0, 10) + "..." : "not set"}`,
      );
      console.log(
        `   ETH Address: ${settings.ethAddress ? settings.ethAddress.substring(0, 10) + "..." : "not set"}`,
      );
      console.log(
        `   Exchange Rates: BTC=$${settings.exchangeRateBtc}, ETH=$${settings.exchangeRateEth}`,
      );
    ...
    console.log(
      "   3. Click on an order and add a transaction hash to complete it",
    );
