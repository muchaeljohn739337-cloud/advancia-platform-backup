...
export async function sendCryptoPayment(cryptoType: "BTC" | "ETH" | "USDT", amount: number, toAddress: string) {
...
async function verifyBitcoinTransaction(txHash: string, expectedAmount: number, expectedAddress: string) {
  ...
  const matchingOutput = tx.outputs.find(
    (out) => out.addresses.includes(expectedAddress) && out.value >= expectedAmount * 1e8, // Convert BTC to satoshis
  );
