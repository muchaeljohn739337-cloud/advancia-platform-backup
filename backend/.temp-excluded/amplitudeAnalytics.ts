 ...
 */
router.post("/amplitude/track", authenticateToken, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get("/amplitude/user-journey/:userId", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const journeyStages = {
      registered: events.some(e => e.eventName === 'user_registered') || !!user,
      ...
      firstTransaction: events.some(e => e.eventName === 'transaction_completed'),
      firstCryptoPurchase: events.some(e => e.eventName === 'crypto_purchase'),
      firstWithdrawal: events.some(e => e.eventName === 'withdrawal_completed'),
      powerUser: events.filter(e => e.eventName === 'transaction_completed').length >= 5
    };

    // Calculate engagement metrics
    const engagement = {
      totalEvents: events.length,
      sessionCount: new Set(events.map(e => e.sessionId).filter(Boolean)).size,
 ...
 */
router.get("/amplitude/funnels", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const segmentFilter = segment ? buildSegmentFilter(segment as string) : {};
    ...
    const mainFunnel = {
      ...
      period: { start, end },
      steps: [
        { name: 'User Registration', count: registrations, conversionRate: 100 },
        {
          name: 'Email Verification',
          count: verifications,
          conversionRate: registrations > 0 ? Number(((verifications / registrations) * 100).toFixed(1)) : 0
        },
        {
          name: 'First Transaction',
          count: firstTransactions,
          conversionRate: verifications > 0 ? Number(((firstTransactions / verifications) * 100).toFixed(1)) : 0
        },
        {
          name: 'Power User (5+ txns)',
          count: repeatUsers,
          conversionRate: firstTransactions > 0 ? Number(((repeatUsers / firstTransactions) * 100).toFixed(1)) : 0
        }
      ]
    };
    ...
    const cryptoFunnel = {
      ...
      steps: [
        ...
        { name: 'Crypto Wallets Created', count: cryptoUsers, conversionRate: registrations > 0 ? Number(((cryptoUsers / registrations) * 100).toFixed(1)) : 0 }
      ]
    };
    ...
    res.json({
      ...
      summary: {
        ...
        conversionRate: mainFunnel.steps[mainFunnel.steps.length - 1].conversionRate,
        highValueUsers,
 ...
 */
router.get("/amplitude/cohorts", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
...
function calculateOnboardingCompletion(stages: any): number {
  const requiredStages = ['registered', 'emailVerified', 'profileCompleted', 'firstLogin'];
...
function buildSegmentFilter(segment: string): any {
  ...
  switch (segment) {
    case 'new_users':
      return { user: { createdAt: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } } };
    case 'power_users':
...
async function calculateAvgTransactions(start: Date, end: Date): Promise<number> {
...
function findBestCohort(cohorts: any[]): any {
  return cohorts.reduce((best, current) =>
    (current.retained_30d / current.cohort_size) > (best.retained_30d / best.cohort_size) ? current : best
  );
}
...
function calculateRetentionTrend(cohorts: any[]): string {
  ...
  const recentAvg = recent.reduce((sum, c) => sum + (c.retained_30d / c.cohort_size), 0) / recent.length;
  const olderAvg = older.length > 0 ? older.reduce((sum, c) => sum + (c.retained_30d / c.cohort_size), 0) / older.length : recentAvg;
...
function generateCohortRecommendations(cohorts: any[]): string[] {
  ...
  if (bestCohort) {
    recommendations.push(`Study ${bestCohort.cohort_month} cohort for successful patterns`);
  }
