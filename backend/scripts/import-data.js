  ...
  try {
    ...
    for (const setting of data.adminSettings || []) {
      try {
        ...
        console.log(
          `  ⚠️  Error importing admin setting ${setting.id}: ${error.message}`,
        );
        stats.errors++;
      }
    }
    ...
    for (const user of data.users || []) {
      try {
        const { transactions, cryptoOrders, cryptoWithdrawals, ...userData } =
          user;
    ...
    for (const transaction of data.transactions || []) {
      try {
        ...
        console.log(
          `  ⚠️  Error importing transaction ${transaction.id}: ${error.message}`,
        );
        stats.errors++;
      }
    }
    ...
    for (const order of data.cryptoOrders || []) {
      try {
        ...
        console.log(
          `  ⚠️  Error importing order ${order.id}: ${error.message}`,
        );
        stats.errors++;
      }
    }
    ...
    for (const withdrawal of data.cryptoWithdrawals || []) {
      try {
        ...
        console.log(
          `  ⚠️  Error importing withdrawal ${withdrawal.id}: ${error.message}`,
        );
        stats.errors++;
      }
    }
    console.log(
      `  ✅ Imported ${stats.cryptoWithdrawals} crypto withdrawals\n`,
    );
  } catch (error) {
...
if (!jsonFilePath) {
  ...
  console.log(
    "Example: node import-data.js ../backups/2025-10-16/data-export.json\n",
  );
  process.exit(1);
}
...
importData(jsonFilePath).catch(console.error);
