...
router.post("/email-signup", async (req: Request, res: Response) => {
  try {
    ...
    const magicLink = `${process.env.FRONTEND_URL || "https://www.advanciapayledger.com"}/auth/verify-signup?token=${signupToken}`;
...
router.get("/verify-signup/:token", async (req: Request, res: Response) => {
  try {
    ...
    if (user.emailSignupTokenExpiry && user.emailSignupTokenExpiry < new Date()) {
      return res.status(400).json({ error: "Signup link has expired. Please register again." });
    }
    ...
    logger.info(`Email verified for user: ${user.email} - Awaiting admin approval`);
 ...
 */
router.post("/login-with-approval-check", async (req: Request, res: Response) => {
  try {
    ...
    if (!user.approved) {
      return res.status(403).json({
        ...
        message: "Your account is pending admin approval. You will be notified when approved.",
        email: user.email,
