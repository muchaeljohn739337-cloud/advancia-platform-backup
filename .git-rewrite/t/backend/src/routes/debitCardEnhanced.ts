 ...
 */
router.get("/my-cards", authenticateToken, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/customize/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
 ...
 */
router.post("/set-pin/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { cardId } = req.params;
 ...
 */
router.post("/freeze/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { cardId } = req.params;
 ...
 * Set spending limits
 */
router.post("/set-limits/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { cardId } = req.params;
 ...
 */
router.get("/transactions/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { cardId } = req.params;
 ...
 */
router.post("/request-physical/:cardId", authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    const { cardId } = req.params;
    ...
    res.json({
      success: true,
      message: "Physical card request submitted. Admin will review and process your request.",
      estimatedDelivery: expedited ? "3-5 business days" : "7-10 business days",
    });
  } catch (error) {
