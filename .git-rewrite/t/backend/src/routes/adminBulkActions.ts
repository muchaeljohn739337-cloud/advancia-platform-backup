 ...
 */
router.post("/activate-users", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (userIds.length > 100) {
      return res.status(400).json({ error: "Maximum 100 users per bulk operation" });
    }
    ...
    res.json({
      success: true,
      message: `${result.count} users ${active !== false ? "activated" : "deactivated"}`,
      count: result.count,
    });
  } catch (error) {
 ...
 */
router.post("/delete-users", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (userIds.length > 50) {
      return res.status(400).json({ error: "Maximum 50 users per delete operation" });
    }
    ...
    res.json({
      success: true,
      message: `${userIds.length} users ${permanent ? "permanently deleted" : "deactivated"}`,
 ...
 */
router.post("/assign-role", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (!validRoles.includes(role)) {
      return res.status(400).json({ error: "Invalid role. Use USER, ADMIN, or MODERATOR" });
    }
 ...
 */
router.post("/send-email", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (userIds.length > 500) {
      return res.status(400).json({ error: "Maximum 500 users per email operation" });
    }
 ...
 */
router.post("/adjust-balance", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
      ...
      try {
        ...
        await prisma.transaction.create({
          data: {
            ...
            status: "completed",
            description: reason || `Admin balance ${operation}: ${adjustAmount}`,
          },
        });
 ...
 */
router.post("/verify-email", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/export-users", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/reset-password", authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { userIds } = req.body;
    ...
    const requests = await Promise.all(
      users.map(async (user) => {
        const token = `RESET_${user.id}_${Date.now()}_${Math.random().toString(36).substring(7)}`;
